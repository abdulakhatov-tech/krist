...
const ListItem: React.FC<PropsI> = ({ sub, length }) => {
	return (
		<CarouselItem
			className={cn(
				"border p-5 rounded-sm ml-4 hover:bg-[#DB4444] hover:text-white hover:scale-[0.97] transition-all duration-75",
				length === 2 && "basis-1/2",
		...
		>
			...
			>
				<div className="flex flex-col gap-4 items-center justify-center w-full ">
					{sub.image_url ? (
						<Image src={sub.image_url} alt={sub.name} width={56} height={56} />
					) : (
